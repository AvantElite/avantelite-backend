...
router.get('/stats', asyncHandler(async (req, res) => {
    ...
    const [mensualRaw]    = await pool.query("SELECT DATE_FORMAT(fecha_creacion,'%b') as mes, COUNT(*) as recibidos, SUM(CASE WHEN leido=1 THEN 1 ELSE 0 END) as leidos FROM contactos WHERE fecha_creacion > DATE_SUB(NOW(), INTERVAL 12 MONTH) GROUP BY MONTH(fecha_creacion) ORDER BY fecha_creacion ASC");
    const [diarioRaw]     = await pool.query("SELECT DATE_FORMAT(fecha_creacion,'%a') as dia, COUNT(*) as contactos FROM contactos WHERE fecha_creacion > DATE_SUB(NOW(), INTERVAL 7 DAY) GROUP BY DATE(fecha_creacion) ORDER BY fecha_creacion ASC");
